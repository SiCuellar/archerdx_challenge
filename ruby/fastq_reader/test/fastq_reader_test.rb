...
class FastqReaderTest < Minitest::Test
  ...
  def test_it_can_return_percentage_of_sequences_over_30
    dir_name = ("read3")
    fastq = FastqReader.new(dir_name)
    assert_equal [["read3/sample_2.fastq", 25.0], ["read3/sample.fastq",50.0]], fastq.percent_seq_over_30
  end

  def test_it_can_return_clean_data
    dir_name = ("read3")
    fastq = FastqReader.new(dir_name)
    assert_equal [["read3/sample_2.fastq", 25.0], ["read3/sample.fastq", 50.0]], fastq.data_display
  end
end
